...
For ML-KEM the indicator changes to 0 when the "ikme" parameter is set as
external IKME is not allowed in FIPS approved-only mode.

This parameter is also described in L<provider-kem(7)>.
